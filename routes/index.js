...
router.get("/show-the-json-two", (req, res) => {
  ...
  pdfParser.loadPDF("./files/NewMidAtlanticShowcase.pdf");
});

router.get("/show-the-json-three", (req, res) => {
  let pdfParser = new PDFParser();
  ...
  pdfParser.on("pdfParser_dataReady", pdfData => {
    ...
    pdfData.formImage.Pages.forEach((thePageObject, index) => {
      ...
      if (index > 0) {
        ...
        thePageObject.Texts.forEach(theText => {
          ...
          // if (theText.x == 3.563 && theText.y == "2.869") {
          //   var texta = decodeURIComponent(theText.R[0].T);

          //   theObj.playerName = texta;
          // }
          ...
          if (theText.x == "3.563") {
            ...
            if (texta) {
              if (validator.isEmail(texta)) {
                email = texta;
                
                theObj.clubTeamName = theTournamentName;
                theObj.email = texta;
                tournament.push(theObj);
                theObj = {};
              } else {
                theObj.playerName = texta;
              }
    ...
    tournament.forEach(t => {
      data.push({
        "CLUB TEAM NAME" : t.clubTeamName,
        "PLAYER NAME" : t.playerName.split('(')[0],
        "GRAD YEAR" : t.playerName.split("(")[1].split(")")[0],
        "EMAIL": t.email
      })
    })
    res.send(data);
  ...
  pdfParser.loadPDF("./files/NewNavyRivalryChallenge.pdf");
});
...
function writeToExcel(tournamentObject) {
  ...
  jsonexport(tournamentObject, function(err, csv) {
